...
## ⭐ The face of a LIVE rack unit, since 2026-09-06. Until then a live row was a
## full-width bar FILLED with its head value's threshold colour — nine solid
## saturated-green slabs stacked in a dark frame, a wall of green on which a
## single orange bar had nothing to stand out from. Now the body is this fixed
## dark and only a 6 px stripe on the left carries the state (the acc_/chip_
## idiom the wall and the shelf already use), so green is quiet and orange/red
## is the only loud thing on the rack. The value was MEASURED, not picked: it
## sits midway between FRAME and RETIRED in luminance (1.10:1 to each — lighter
## than the frame so a unit reads as an object in a rack, darker than a retired
## slab so the two never pass for one another), and it is as LIGHT as the body
## can go while every threshold colour the rows can show stays above WCAG AA on
## it: Grafana `red` #F2495C is the weakest pair at 4.62:1 (green 7.4, orange
## 7.7, yellow 12.2, `text` 10.4, the estimate blue 7.8, INK 13.7); one step
## lighter (#20232a) already drops red to 4.41. So no threshold colour needs an
## on-dark variant — the standard reds/oranges are the theme's dark-background
## set and this body keeps them in their working range.
UNIT      = "#1c1f26"
## A 1 px bezel edge. The old border was translucent BLACK, which on a bright
## fill drew a dark rim and on this body would draw nothing against the frame.
UNIT_EDGE = "#2e3138"
## Outline of an EMPTY U — 1.08:1 against the frame, deliberately at the edge
## of visibility: the empty slots must read as rack rails, not as content.
SLOT_EDGE = "#1c1e23"
## The state stripe on a live unit. Chips use 4 px on a 22-26 px body; a 17 px
## U row is shorter, so its stripe is a little wider to keep the same weight.
ACC_W     = 6
...
## ⭐ FIXED 2026-09-06 — salt/pepper "mem" now means what qui/quo/qua "mem" means.
## UCD-SNMP `memAvailReal` is MemFree and EXCLUDES buffers and cache, while
## node-exporter's `MemAvailable` (mem() above) INCLUDES reclaimable cache. The
## twin puts all five in ONE column under ONE 80/92 band, so the two x3550s read
## systematically high for the same real pressure: on the day this was corrected,
## salt went 55.2 % -> 43.1 % and pepper 51.0 % -> 36.5 % against unchanged load.
## `memBuffer`/`memCached` were added to the exporter's ucd module the same day
## (they were already inside the 1.3.6.1.4.1.2021.4 walk, just never emitted).
##
## ⛔ `sum without(__name__)(...)` rather than the literal three-term addition, and
## the reason is the whole point of this board: a three-term sum needs ALL THREE
## series, so the day one OID stops answering the panel goes BLANK, and a blank
## reads as "no data yet" rather than as a fault. The sum adds whichever of the
## three exist, so a missing OID degrades to the old MemFree number instead of to
## nothing. Both forms return the same value while all three are present — checked.
##
## ⚠️ Two honest caveats. `memBuffer` reads exactly 0 on both hosts; that is real,
## not broken — they are TrueNAS SCALE, where ZFS keeps almost nothing in the page
## cache, so `memCached` carries the entire correction. And the ZFS ARC itself is
## kernel slab, NOT counted in memCached, so even this figure understates
## reclaimable memory on these two. It is much closer to the DL360s than MemFree
## was; it is not identical, and nothing here can make it so.
def smem(i): return ('100*(1- sum without(__name__)('
                     f'{{__name__=~"memAvailReal|memBuffer|memCached",instance="{i}"}}'
                     f') / memTotalReal{{instance="{i}"}})')
...
STYLE = {
    ...
    ## ⛔ EVERY watt style below is `suffix: W`, NEVER Grafana's `watt` unit
    ## (user, 2026-09-06). `watt` is SI-scaled, so at 1000 it silently switches
    ## to kW and dec(0) then rounds to ONE significant digit: the two rack
    ## totals rendered "1 kW" (metered, 1080 W) and "2 kW" (UPS out, 1645 W) —
    ## a 565 W spread shown as two adjacent integers, and the whole POWER block
    ## is a subtraction chain (ups_out − metered = residual − estimate = err)
    ## that stops being checkable by eye the moment two of its terms carry
    ## different units. Measured 7d maxima: wall plug 1937.7, ups_out 1644.5,
    ## metered 1276.8 — so four digits is the ceiling and "1938 W" (6 chars) is
    ## both NARROWER than "1.94 kW" (7) and 10x more precise. Pinning is free
    ## for the six styles that never reach 1000: `watt` does not scale below
    ## it, so their rendering is byte-identical — they are pinned anyway so a
    ## future load increase cannot re-introduce the trap silently.
    ...
    "watt":  [th([st(None, "green"), st(1850, "yellow"), st(2450, "orange")]), unit("suffix: W"), dec(0)],
    ...
    "wattsm": [th([st(None, "green"), st(400, "yellow")]), unit("suffix: W"), dec(0)],
    ...
    "wattu": [th([st(None, "text")]), unit("suffix: W"), dec(0)],
    ...
    "wattp": [th([st(None, "red"), st(1, "green")]), unit("suffix: W"), dec(0)],
    ...
    "watte": [th([st(None, "#8ab4f8")]), unit("suffix: W"), dec(0)],
    ...
    "wattr": [th([st(None, "green"), st(450, "yellow"), st(600, "orange")]), unit("suffix: W"), dec(0)],
    ...
    "werr":  [th([st(None, "orange"), st(-150, "yellow"), st(-75, "green"),
                  st(75, "yellow"), st(150, "orange")]), unit("suffix: W"), dec(0)],
...
## ⭐ Empty U's are drawn as EMPTY RACK, not as void (2026-09-06). The gaps are
## physically accurate — U40, 36-38, 32-33, 21, 19, 17, 15, 13, 6-8 really are
## open — but flat frame colour between the units read as "nothing here" rather
## than "an open slot in a rack". One outline per empty U, computed from the
## map so the slots follow the units: a hairline box at SLOT_EDGE gives each
## open U its rails and its top/bottom seam at the U pitch, which is what the
## eye uses to count units. ⛔ ONE element per U, never one per mounting hole —
## that would be three per rail per U and would triple the element count for a
## detail nobody can see from across the cellar. Drawn BEFORE the units so a
## unit always sits on top; they never overlap anyway, but the order is what
## makes that a non-issue rather than a coincidence.
MON_U = (24, 31)   # the monitor's U range; drawn in its own section below
occupied = {u for a, b, *_ in RACK for u in range(a, b + 1)} | set(range(MON_U[0], MON_U[1] + 1))
for u in range(1, 43):
    if u not in occupied:
        rect(f"slot_u{u}", DEV_L, u_top(u), DEV_W, UPX, border=SLOT_EDGE, bw=1)
...
for a, b, kind, name, sub, vals in RACK:
    ...
    if kind == "live":
        head = vals[0][0]
        # ⭐ QUIET WHEN HEALTHY, LOUD WHEN NOT (2026-09-06). A live unit is a
        # fixed dark body (UNIT) with a 6 px state stripe down its left edge —
        # the same acc_/chip_ language the wall chips and the shelf behind the
        # screen already speak, so the rack rows stop being the one idiom on the
        # board that shouts. Until 2026-09-06 the whole bar was filled with the
        # head value's colour: nine bright green slabs in a dark frame, and
        # since everything healthy was maximally loud, nothing was.
        # ⚠️ The stripe carries the HEAD value's state only (bgfield=head), so a
        # scrape gap does not redden it — it UN-PAINTS it, and every value in
        # the row goes blank with it. The body is a FIXED fill now, so the unit
        # stays drawn with its name over three empty readouts, which is the
        # better failure: before, the whole bar vanished and read as an empty
        # U — exactly what the slot outlines above now look like. ⛔ Do NOT
        # "fix" this with `or vector(0)` on cpu/mem/watt: a guarded CPU renders
        # a confident green "0 %" for a host that is OFF, which is strictly
        # worse than blank — blank at least looks wrong. The right fix is a
        # distinct "stale/absent" treatment (a colour that means "no reading",
        # the way watte means "modelled"), and nobody has built it yet.
        # Recorded 2026-09-05.
        for f, e, sty in vals:
            target(f, e); override(f, STYLE[sty])
        rect(f"acc_{head}", DEV_L, top, ACC_W, h, bgfield=head)
        # Size 12 for the NAME and 11 for the values is the row's whole
        # hierarchy: the name is the anchor you find the row by, the values are
        # what you read — and they carry colour, which is louder than a point
        # of size. The caption stays 9/DIM, the same secondary ink every hint
        # on the board uses (U numbers, wall notes, retired subs); it is there
        # to be read up close, not from across the cellar. Nothing under 9.
        rect(f"dev_{head}", DEV_L + ACC_W, top, DEV_W - ACC_W, h, text="  " + name,
             size=12, bg=UNIT, color=INK, border=UNIT_EDGE, bw=1)
        ...
        for idx, (col, w) in enumerate(slots[len(slots) - len(vals):]):
            # ⭐ Every value is coloured by ITS OWN field. Until 2026-09-06 the
            # values were fixed near-black ink (the only thing legible on a
            # bright fill), so the styles on the 2nd/3rd/4th values — memory %,
            # watts, error rate, UPS runtime — computed a threshold colour that
            # was then thrown away: only the head value's state ever reached a
            # pixel, and only through the bar fill. On the dark body light ink
            # is the readable choice, so each reading now shows its own state,
            # and a fault appears TWICE — the stripe says "this unit", the
            # number says "this reading". Value 0 also drives the stripe; it is
            # shown here anyway so a single-value row still reads.
            rect(f"v{idx}_{head}", DEV_L + col, top, w, h, field=vals[idx][0],
                 size=11, colorfield=vals[idx][0], align="right")
    else:
        ...
        ## dropped. Same COL_SUB column and the same DIM/9 as the live rows'
        ## caption (they carried a dark tint until 2026-09-06, when the live
        ## bars were still coloured fills). Retired rows carry an empty `sub`,
        ## so they emit nothing and are unchanged.
        if sub:
            rect(f"sub_{a}_{b}", DEV_L + COL_SUB, top, 84, h, text=sub, size=9,
                 color=DIM, align="right")

# ------ the monitor, and the machines stacked in the void behind it ---------
mon_top, mon_h = u_rect(*MON_U)
...
rect("wall_sub", CHIP_L, 44, CHIP_W, 16, text="on the wall / a shelf — not racked",
     size=9, color=DIM)
## ⭐ The whole wall group is lifted by ONE offset (2026-09-06). The header
## block ends at y=60 and the first chip sat at y=128 — a 68 px hole with only
## the bare wall line in it, so the header floated above a gap instead of
## capping its column. The chips' y values in WALL are their real relative
## positions and are kept verbatim: the lift moves the GROUP, so the spacing
## between bpi-r4, the GPS adapter and the plug is unchanged, only the block
## starts 8 px under the header — the same +8 rhythm the right-hand column
## uses between a section label and its first row. The wall line still starts
## at 66 and now ENDS where the last chip ends (+8), derived rather than typed,
## so it follows the group if a chip is added or the lift changes.
WALL_LIFT = 128 - 68
wall_end = max(y - WALL_LIFT + (22 if s else 26) for y, *_, s in WALL) + 8
rect("wall", WALL_X, 66, 3, wall_end - 66, bg=WALLC)
...
rect("gps_note", CHIP_L + 18, 358 - WALL_LIFT, CHIP_W - 18, 14,
